...
- **并行读取**：当需要修改多个文件时，同一条消息内一次性并行 read 所有目标文件，不要逐个串行读取。读完后再批量 edit/write
- 快速直接——不研究、不委派、不多步研究/规划；允许最小执行顺序
- 被要求时编写或更新测试，尤其涉及测试文件、fixture、mock 或测试辅助的有界任务
- 验证策略（场景区分）：
  - **多文件并行修改 Wave 中**（orchestrator 委派指令注明"本轮只修改不验证"时）：仅修改文件，不运行编译/测试。报告时在 <verification> 中注明"验证待统一执行"
