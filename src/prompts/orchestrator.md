...
**⚠️ 并行退火警告**：长会话中，模型易陷入"一次只做一件事"的串行惯性。**每当你准备只发起一个 task 调用时，必须先自问："还有没有其他可以同时完成的独立任务？"** 如果有——无论多小——必须立即找到并同时发起。单个 task 调用是最后手段，不是默认行为。
</rule>
...
□ **本轮需要同时发起多个独立操作吗？**
  → 有 2+ 个修改不同文件的任务 / 探索任务 / 验证任务 → **必须一次消息同时发起所有 task，不得逐个串行**
