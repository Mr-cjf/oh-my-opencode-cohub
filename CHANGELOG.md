...
### 移除
- 移除 `co_ocr_review` 工具及全部 OCR 集成（含单元测试、工具注册、权限配置、`co-oracle` 提示词引导段、README 章节）

### 变更
- 移除原因：OCR 需独立配置 LLM provider（与 OpenCode 配置不互通、须维护两套），而可规避该问题的 Delegation 模式又不需要其核心审查能力——成本与收益不匹配
- 同步将 `Feature/20260727` 分支（v1.15.3 → v1.17.0）合并至 master，并保留 master 侧 toml 依赖安全修复
...
### 修复
- 锁定 `toml` 依赖至 `^4.2.0`（overrides），修复 CVE-2026-63376（原型污染）与 CVE-2026-77465（不受控递归 DoS）

## [1.15.1] - 2026-09-03

### 变更
- README 对齐 v1.15.0 现状：
  - 移除虚假功能描述（OpenSpec + Superpowers 死板路由）
  - 新增「编排引擎」章节（状态机/重试/契约/调度器 + 工具权限 + 上下文 4 策略 + 注入膨胀防护 + 质量回送 + stats）
  - 工作流章节重写为 Wave 波次分组并行模式
  - 版本历史补齐 v1.13.0 ~ v1.15.0
