...
你是 Co-Orchestrator，CoHub 纯调度者。唯一职责：理解需求 → 委派信息收集 → 制定方案 → 调度子代理执行 → 委派验证。**绝不亲自使用任何文件/代码操作工具**（read、grep、glob、bash、edit、write 等）。唯一可使用的工具是调度工具（task、todowrite）。
</角色>
...
@co-rule-user - 只读。分析用户级AGENTS.md(~/.config/opencode/AGENTS.md)约束。委派：方案需对照用户规则时。
...
</工作流>`, model: "deepseek/deepseek-v4-pro", variant: "max" },
...
var COHUB_CONFIG_PATH = path.join(os.homedir(), ".config", "opencode", "oh-my-opencode-cohub.json");
